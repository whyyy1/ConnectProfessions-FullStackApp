...
function SearchPage() {
    ...
    const getUsers = async () => {
      ...
      try {
        const response = await axios.get(
          `https://fp-server-ox4k.onrender.com/cp/users`
        );
  ...
  return (
    <div className="flex flex-col p-5 bg-purple-200 h-screen">
      <Card className="h-full w-full">
        <CardHeader
          floated={false}
          shadow={false}
          className="rounded-lg text-center h-3/4"
        >
        ...
        <div className="flex gap-2">
          ...
          <Button
            onClick={() => setFilterState(userData)}
            variant="outlined"
            size="sm"
          >
            view all
